...
def userRegister(request):
    try:
        ...
        if not name and not contact_no and not password and not firebase_token:
            return Response(
                {"message": "Please provide valid name, contact_no, and password."},
                status=400,
            )
        ...
        manageNotifications(
            ...
            f"A new user named {name} has registered. Please review their details in the admin panel.",
        )
        return Response(
            {"message": "User register successfully.Wait for Approval."}, status=200
        )
...
def userLogin(request):
    try:
        ...
        if not (username and password):
            return Response(
                {"message": "Please provide a valid username and password."}, status=400
            )
            ...
            return Response(
                {
                    ...
                    "token": token,
                    "userData": getUsersData(user_data),
                },
                status=200,
            )
        else:
...
def getUserDataList(request, userId):
    try:
        if not userId:
            return Response(
                {"message": "Please provide appropiat User id or all"}, status=400
            )
        ...
        if userId == "all":
            ...
            return Response(
                {"message": "User Data fecthed", "data": getUsersData(userData)},
                status=200,
            )
...
def userApproval(request):
    try:
        ...
        if userId is None or approval_status is None:
            return Response(
                {"message": "Please provide valid userId and approvalStatus."},
                status=400,
            )
        ...
        if approval_status == "1":
            manageNotifications(
                "User",
                user_instance.expo_go_token,
                f"Approval Confirmation for {user_instance.name}",
                f"Congratulations, {user_instance.name}! Your registration has been approved by the admin. Welcome to Vardhaman!",
            )
        ...
        if approval_status == "2":
            ...
            manageNotifications(
                "User",
                user_instance.expo_go_token,
                f"Registration Rejection for {user_instance.name}",
                f"We regret to inform you that your registration has been rejected. If you have any questions or concerns, please don't hesitate to reach out to us.",
            )

        user_instance.save()
        return Response(
            {"message": f"User{status_value[approval_status]}successfully"}, status=200
        )
    except json.JSONDecodeError:
        return Response(
            {"message": "Invalid JSON data in the request body."}, status=400
...
def getAllProductsList(request):
    try:
        ...
        return Response(
            {"message": "Product Data fecthed", "data": productList}, status=200
        )
    except Exception as e:
        logger.error(f"User Register Exception : {str(e)}")
        return Response({"message": str(e)}, status=500)
...
def searchProducts(request):
    try:
        ...
        if not name:
            return Response(
                {"message": "Product Data searched", "data": productList}, status=200
            )

        productData = Products.objects.filter(
            (
                Q(product_name_eng__icontains=name)
                | Q(product_name_guj__icontains=name)
                | Q(product_name_hin__icontains=name)
            )
        ).exclude(is_delete="1")
...
def generateOrder(request):
    try:
        ...
        if not userId or not productList:
            return Response(
                {
                    "message": "Please provide userId or productList.",
                },
                status=400,
            )
        ...
        if (
            not userdata.name
            or not userdata.email
            or not userdata.contact_no
            or not userdata.address
        ):
        ...
        for i in productList:
            try:
                ...
                amount = float(productdata.product_price) * int(
                    i["qty"]
                )  # count the total amount of product
                product_tax = (
                    float(productdata.product_gst_rate) / 2
                )  # count the tax for cgst and sgst
                gst_cal_amount = (
                    (float(productdata.product_gst) / 2) * int(i["qty"])
        ...
        manageNotifications(
            "User",
            userdata.expo_go_token,
            f"Order Created by {userdata.name}",
            f"A new order has been created by {userdata.name}. Please review the details promptly.",
        )
...
def listOfOrders(request, userId):
    try:
        ...
        if getUser.role == "1":
            orderData = (
                Order.objects.filter(created_at__range=[dateRange, timezone.now()])
                .exclude(customer_id=userId)
                .order_by("-created_at")
            )
        else:
            orderData = Order.objects.filter(
                customer_id=userId, created_at__range=[dateRange, timezone.now()]
            ).order_by("-created_at")
                ...
                if (ls.order_id.id) == (item.id):
                    ...
                    try:
                        itemObj["product_image"] = (
                            ls.product_id.product_image.url
                            if ls.product_id.product_image
                            else ""
                        )
                    except:
                        itemObj["product_image"] = ""
...
def generatePDF(orderId):
    try:
        try:
            ...
            return Response(
                {"message": "Order matching query does not exist."}, status=400
            )

        ordersList = Order_data.objects.filter(order_id=orderId, status="1")
        taxes_order = order_taxes.objects.filter(order_id=orderId)

        totalAmountWithGST = float(orders.total_amount) + (
            calculate_sgst(float(orders.total_amount), 4) * 2
        )
...
def updateOrders(request, orderId):
    try:
        ...
        if not orderList:
            return Response(
                {"message": "Please provide appropiat orderList"}, status=400
            )
        ...
        for lis in order_data_list:
            ...
            if lis.order_id == orderVal:
                ...
                taxObj["taxable_amount"] = float(lis.product_id.product_price) * float(
                    lis.qty
                )
                taxObj["tax_amount"] = float(lis.product_id.product_gst) * float(
                    lis.qty
                )
        ...
        manageNotifications(
            "User",
            userName.expo_go_token,
            f"Order Confirmation for {userName.name}",
            f"Your order has been confirmed by the admin. Thank you for your purchase!",
        )
